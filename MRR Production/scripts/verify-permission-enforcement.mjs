...
async function makeUser(role) {
  ...
  const { data: au, error } = await admin.auth.admin.createUser({
    ...
    password,
    email_confirm: true,
    user_metadata: { full_name: `Perm ${role}` },
  });
  if (error) throw error;
  const uid = au.user.id;
  await admin
    .from("memberships")
    .insert({ user_id: uid, company_id: testCompanyId, role, active: true });
  await admin
    .from("profiles")
...
async function expectDeny(label, client, from, to) {
  ...
  check(
    `${label}  (${from} → ${to} BLOCKED)`,
    denied,
    ok
      ? "TRANSITION SUCCEEDED — NOT ENFORCED"
      : landed !== from
        ? `status changed to '${landed}' anyway`
        : "",
  );
}
...
async function expectInsert(label, client, shouldAllow) {
  ...
  const { data, error } = await client
    .from("jobs")
    .insert({ id, company_id: testCompanyId, status: "draft", title: "Perm Test Insert" })
    .select("id");
  ...
  if (shouldAllow)
    check(
      ...
      ok,
      error ? `BLOCKED: ${error.code} ${error.message}` : "affected 0 rows",
    );
...
try {
  ...
  console.log(
    `  Seeded role_permissions mirroring production for: ${Object.keys(ROLE_PERMS).join(", ")}`,
  );
  ...
  await admin.from("user_permission_overrides").upsert(
    {
      company_id: testCompanyId,
      user_id: users.employee.uid,
      overrides: { jobs_close: true },
    },
  ...
  await admin.from("user_permission_overrides").upsert(
    {
      company_id: testCompanyId,
      user_id: users.coordinator.uid,
      overrides: { jobs_pull: false },
    },
    { onConflict: "company_id,user_id" },
  );
  ...
  await admin
    ...
    .delete()
    .eq("company_id", testCompanyId)
    .eq("user_id", users.coordinator.uid);
  ...
  check(
    "service_role bypasses enforcement",
    !svcErr && (await statusOf(svcJob)) === "closed",
  ...
  const { error: edErr } = await coord
    .from("jobs")
    .update({ title: "Renamed by coordinator" })
    .eq("id", editJob)
    .select("id");
  check(
    "coordinator CAN edit a job's title (no status change)",
    !edErr,
    edErr ? `BLOCKED: ${edErr.message}` : "",
  );
} catch (err) {
