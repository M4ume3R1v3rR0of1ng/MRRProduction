...
try {
  ...
  const { data: mrr } = await admin
    .from("companies")
    .select("id, name")
    .eq("slug", "maumee-river-roofing")
    .single();
  ...
  for (const t of TENANT_TABLES) {
    const { count } = await admin
      ...
      .eq("company_id", mrr.id);
    realCounts[t] = count ?? 0;
  }
  const totalReal = Object.values(realCounts).reduce((a, b) => a + b, 0);
  console.log(
    `  Rows belonging to ${mrr.name}: ${totalReal} across ${TENANT_TABLES.length} tables`,
  );
  if (totalReal === 0) {
    console.error(
  ...
  await admin
    .from("profiles")
    .update({ active_company_id: testCompanyId, active: true })
    .eq("id", testUserId);
  ...
  for (const t of TENANT_TABLES) {
    ...
    check(
      `${t} — sees 0 of ${mrr.name}'s ${realCounts[t]} rows`,
      leaked === 0,
  ...
  for (const t of TENANT_TABLES) {
    ...
    check(
      `${t} — no foreign rows in a bare select *`,
      foreign === 0,
  ...
  check(
    "jobs — cross-tenant INSERT is rejected by RLS",
    rlsBlocked,
  ...
  const { data: upd } = await user
    ...
    .select("id");
  check(
    "jobs — cross-tenant UPDATE affects 0 rows",
    (upd || []).length === 0,
  ...
  check(
    "jobs — cross-tenant DELETE affects 0 rows",
    (del || []).length === 0,
  ...
  const { data: esc } = await admin
    ...
    .single();
  check(
    "cannot make self platform_admin",
    esc?.is_platform_admin !== true,
  ...
  const { data: hop } = await admin
    ...
    .single();
  check(
    "cannot point self at another company",
    hop?.active_company_id !== mrr.id,
    hop?.active_company_id === mrr.id ? "HOPPED TENANTS" : "",
  );

  const { error: swErr } = await user.rpc("set_active_company", { target: mrr.id });
  check(
    "set_active_company() rejects a company they're not in",
    !!swErr,
  ...
  check(
    "cannot read the other company's row",
    (coRead || []).length === 0,
  ...
  const { data: ownSec, error: ownErr } = await user
    .from("company_secrets")
    .select("integrations")
    .eq("company_id", testCompanyId);
  check(
    "company_secrets not readable for own company",
    !!ownErr || (ownSec || []).length === 0,
    (ownSec || []).length > 0 ? "OWN SECRETS READABLE" : ownErr ? `(blocked: ${ownErr.code})` : "",
  );
  const { data: mrrSec, error: mrrErr } = await user
    .from("company_secrets")
    .select("integrations")
    .eq("company_id", mrr.id);
  check(
    "company_secrets not readable for another company",
    !!mrrErr || (mrrSec || []).length === 0,
    (mrrSec || []).length > 0
      ? "FOREIGN SECRETS READABLE"
      : mrrErr
        ? `(blocked: ${mrrErr.code})`
        : "",
  );
  ...
  const { error: upOwn } = await user.storage
    .from(BUCKET)
    .upload(ownPath, tinyJpeg, { contentType: "image/jpeg", upsert: true });
  check("can upload into OWN company folder", !upOwn, upOwn?.message || "");

  const { error: upEvil } = await user.storage
    .from(BUCKET)
    .upload(evilPath, tinyJpeg, { contentType: "image/jpeg", upsert: true });
  check(
    "cannot upload into another company's folder",
    !!upEvil,
    upEvil ? "" : "WROTE INTO FOREIGN FOLDER",
  );

  const { data: listEvil } = await user.storage.from(BUCKET).list(mrr.id);
  check(
    "cannot enumerate another company's files",
    !listEvil || listEvil.length === 0,
  ...
  if (!upEvil) {
    try {
      await admin.storage.from(BUCKET).remove([evilPath]);
    } catch {}
  }
  ...
  try {
    await admin
      .from("warehouses")
      .insert({ id: "wtest", name: "Test WH", company_id: testCompanyId });
  } catch {}
  ...
  check(
    "canceled company sees an EMPTY portal",
    (afterKill || []).length === 0,
