...
async function makeUser(role) {
  ...
  await admin
    .from("memberships")
    .insert({ user_id: au.user.id, company_id: coId, role, active: true });
  await admin
    .from("profiles")
    .update({ active_company_id: coId, active: true })
    .eq("id", au.user.id);
...
const statusOf = async () =>
  (await admin.from("jobs").select("status").eq("company_id", coId).eq("id", "atomic_job").single())
    .data?.status;
...
try {
  ...
  const { data: co, error: coErr } = await admin
    .from("companies")
    .insert({ name: "ZZ Atomic Test", slug: SLUG, subscription_status: "active" })
    .select("id")
    .single();
  ...
  for (const [role, permissions] of Object.entries(PERMS)) {
    await admin
      ...
      .upsert({ company_id: coId, role, permissions }, { onConflict: "company_id,role" });
  }
  ...
  const items = [
    { iid: "atomic_a", iname: "Item A", planned: 15, pulled: 0, priceAtPull: 0, pullCost: 0 },
  ];
  ...
  check(
    "a bad item raises instead of silently skipping",
    rejected(rbErr),
    rbErr ? "" : "NO ERROR RAISED",
  );
  check(
    "job status ROLLED BACK to 'approved'",
    (await statusOf()) === "approved",
    `status is '${await statusOf()}' — job would be STRANDED`,
  );
  check(
    "the good item's stock ROLLED BACK to 20",
    (await remOf("atomic_a")) === 20,
  ...
  check(
    "bookkeeper (no jobs_pull) is REJECTED by the trigger",
    rejected(permErr),
    permErr ? "" : "PULL ALLOWED — DEFINER LEAK",
  );
  check(
    "nothing was written on the rejected pull",
    (await statusOf()) === "approved" && (await remOf("atomic_a")) === 20,
  );
  ...
  check(
    "cannot touch another company's job",
    rejected(xErr),
    xErr ? "" : "CROSS-TENANT WRITE SUCCEEDED",
  );
  const { data: after } = await admin
    ...
    .single();
  check(
    "the other company's job is untouched",
  ...
  const { data: done } = await admin
    ...
    .single();
  check(
    "job completed and timestamped",
    done?.status === "completed" && !!done?.completedAt,
