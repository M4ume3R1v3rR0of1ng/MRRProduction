...
export default function InventoryView({
  ...
  const cats = [
    "All",
    ...new Set(inv.map((i) => i.cat).filter(Boolean)),
  ].sort();
  ...
  const handleSaveItem = async () => {
    setSaving(true); // 🔒 Engage interaction barrier
    try {
      ...
      // ... handle success (e.g., alert, modal close)
    } catch (err) {
      console.error("Failed to save item:", err);
    } finally {
      setSaving(false); // 🔓 Release interaction thread
    }
  };
  ...
  const addItem = async () => {
    ...
    await logAction(
      ...
      {
        targetId: item.id,
        warehouseCode: item.warehouse || "WH-A",
        payload: { before: item.qty, adjustment: adjustmentQty },
      },
    );
  ...
  const editItem = async () => {
    ...
    if (error) {
      showToast(
        `Database Error modifying catalog record: ${error.message}`,
        "error",
      );
    } else {
      setInv((p) =>
  ...
  const rcvBatch = async () => {
    ...
    if (error) {
      showToast(
        `Database Error posting receipt batch: ${error.message}`,
        "error",
      );
    } else {
      setInv((p) =>
  ...
  const bulkTotal = bulkItems.reduce(
  ...
  const confirmBulk = async () => {
    ...
    try {
      ...
      showToast(
        `Error logging batch payload operations: ${err.message}`,
        "error",
      );
    }
  };
    ...
    <div>
          ...
          return (
            <div
              ...
              style={{
                ...
                border: `2px solid ${isLow ? C.rd : "transparent"}`,
                cursor: "pointer",
              }}
            >
              {photo ? (
                <div
                  style={{ height: 110, overflow: "hidden", background: C.lg }}
                >
                  <img
                    src={photo}
                    alt={item.name}
                ...
                <div style={{ height: 6, background: sClr(item) }} />
              )}
              <div style={{ padding: 14 }}>
                ...
                >
                  <div>
                    <div
                      style={{ fontWeight: 800, color: C.navy, fontSize: 13 }}
                    >
                      {item.name}
                    </div>
                ...
                >
                  ...
                  {perms.inv_pricing_view ? (
                    <div style={{ textAlign: "right" }}>
                      <div
                        style={{ fontSize: 13, fontWeight: 800, color: C.blue }}
                      >
                        {fm(newestPrice(item))}
                      </div>
                      <div style={{ fontSize: 10, color: C.sub }}>
                        per {item.unit.replace(/s$/, "")}
                      </div>
                    </div>
                  ) : (
                    <div style={{ fontSize: 11, color: C.sub }}>
                ...
                {!photo && (
                  ...
                  >
                    <div
                      style={{
                        height: "100%",
                        background: sClr(item),
                        borderRadius: 2,
                        width: `${Math.min(100, (stock / (item.alrt * 3)) * 100)}%`,
                      }}
                    />
                  </div>
                )}
      ...
      {modal === "detail" && sel && (
        <Modal title={sel.name} onClose={() => setModal(null)} wide>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 13, fontWeight: 800, color: C.navy }}
                    >
                      {v}
                    </span>
                  </div>
                ...
                {perms.inv_edit && (
                  <Btn
                    ...
                    onClick={async () => {
                      if (
                        window.confirm(
                          `Permanently delete ${sel.name} from inventory?`,
                        )
                      ) {
                        const { error } = await supabase
                          .from("inventory")
            ...
            .map((b, i) => (
              <div
                ...
                style={{
                  padding: "10px 14px",
                  background:
                    i === 0 && b.rem > 0 ? "rgba(27,82,184,0.08)" : C.lg,
                  borderRadius: 8,
                  border:
                    i === 0 && b.rem > 0 ? `1.5px solid ${C.blue}` : "none",
                  marginBottom: 8,
                }}
              >
                <div
                ...
                >
                  <div>
                    <div
                      style={{ fontWeight: 700, color: C.navy, fontSize: 12 }}
                    >
                      {i === 0 && b.rem > 0 && (
                        <span style={{ color: C.blue }}>▶ ACTIVE · </span>
                      )}
                      {fd(b.rcvd)}
                      {b.vendor && (
                        <span style={{ color: C.sub }}> · {b.vendor}</span>
                      )}
                      {b.ref && <span style={{ color: C.tl }}> · {b.ref}</span>}
                    </div>
                    <div style={{ fontSize: 11, color: C.sub }}>
                      By: {users.find((u) => u.id === b.by)?.name || "Unknown"}
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div
                    ...
                    {perms.inv_pricing_view && (
                      <div
                        style={{ fontSize: 11, color: C.blue, fontWeight: 700 }}
                      >
                        {fm(b.price)} ea.
                      </div>
                    )}
          ...
          {sel.batches.length === 0 && (
            <p style={{ color: C.sub, fontSize: 13 }}>No batches yet.</p>
          )}
        </Modal>
      )}
      ...
      {modal === "add" && (
        <Modal title="Add New Item" onClose={() => setModal(null)}>
          ...
          <Fld label="Low Alert Threshold">
            <Inp
              type="number"
              value={form.alrt || ""}
              onChange={(e) => setForm({ ...form, alrt: e.target.value })}
            />
          </Fld>
      ...
      {modal === "rcv" && sel && (
        <Modal title={`Receive: ${sel.name}`} onClose={() => setModal(null)}>
          ...
          {perms.inv_pricing_edit ? (
            ...
            >
              Pricing is set by authorized personnel. Price will carry over from
              the last batch.
            </div>
          )}
          <div style={{ display: "flex", gap: 10 }}>
      ...
      {modal === "bulk" && perms.inv_bulk_receive && (
        ...
        >
          ...
          >
            ⭐ <strong>FIFO applied automatically.</strong> Each item gets one
            new batch. Older batches are consumed first.
          </div>
          <div
            style={{
              display: "grid",
          ...
          >
            <Fld label="Date Received *">
              <Inp
                ...
                value={bulkMeta.date}
                onChange={(e) =>
                  setBulkMeta({ ...bulkMeta, date: e.target.value })
                }
              />
            </Fld>
            <Fld label="PO / Order #">
              <Inp
                value={bulkMeta.po}
                onChange={(e) =>
                  setBulkMeta({ ...bulkMeta, po: e.target.value })
                }
            ...
            <Fld label="Vendor / Supplier">
              <Inp
                value={bulkMeta.vendor}
                onChange={(e) =>
                  setBulkMeta({ ...bulkMeta, vendor: e.target.value })
                }
          ...
          <div style={{ display: "flex", gap: 14, marginBottom: 16 }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <Inp
                value={bulkSrch}
                onChange={(e) => setBulkSrch(e.target.value)}
                ...
                {bulkFiltered.map((item) => (
                  ...
                  >
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: C.sub }}>
                        {item.cat} · {tot(item)} {item.unit} in stock
                      </div>
                    </div>
                    <Btn v="primary" sz="sm" onClick={() => addToBulk(item)}>
                      + Add
                    </Btn>
                  </div>
                ))}
            ...
            <div style={{ flex: "0 0 360px" }}>
              ...
              >
                ...
                >
                  Order Items {bulkItems.length > 0 && `(${bulkItems.length})`}
                </div>
                {bulkItems.length > 0 && (
                  <button
                ...
                <>
                        ...
                        >
                          ...
                          <button
                            ...
                            style={{
                              ...
                              // ... rest of your UI primitive styles
                            }}
                          >
                            {saving ? "Processing Query..." : "💾 Save Changes"}
                          </button>
                          <div
                            style={{
                              display: "grid",
                          ...
                          >
                            <div>
                              ...
                              <Inp
                                ...
                                value={b.qty}
                                onChange={(e) =>
                                  updateBulk(b.iid, "qty", e.target.value)
                                }
                                placeholder="0"
                              ...
                              <div style={{ position: "relative" }}>
                                ...
                                {perms.inv_pricing_edit ? (
                                  <Inp
                                    ...
                                    value={b.price}
                                    onChange={(e) =>
                                      updateBulk(b.iid, "price", e.target.value)
                                    }
                                    placeholder="0.00"
                                    style={{
                                      padding: "5px 8px",
                            ...
                            <div
                              style={{ paddingBottom: 2, textAlign: "right" }}
                            >
                              <div
                                style={{
                                  fontSize: 9,
                  ...
                  >
                    <div>
                      ...
                      >
                        {bulkItems.filter((b) => parseFloat(b.qty) > 0).length}{" "}
                        of {bulkItems.length} items with quantities
                      </div>
                    </div>
                    <div
                      style={{ fontWeight: 900, fontSize: 22, color: C.gold }}
                    >
                      {fm(bulkTotal)}
                    </div>
                  </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            >
              ✅ Receive{" "}
              {bulkItems.filter((b) => parseFloat(b.qty) > 0).length > 0
                ? `${bulkItems.filter((b) => parseFloat(b.qty) > 0).length} Items `
                : ""}
              into Warehouse
            </Btn>
          </div>
