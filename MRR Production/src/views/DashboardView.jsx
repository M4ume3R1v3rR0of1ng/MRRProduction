...
export default function DashboardView({
  ...
  const myJobs =
    user.role === "field"
      ? jobs.filter((j) => j.assignedTo === user.id && j.status !== "completed")
      : jobs;
  const newJobs = myJobs.filter(
    (j) => j.newForAssigned && j.assignedTo === user.id,
  );
      ...
      {perms.maint_manage && pendingReqs.length > 0 && (
        ...
        >
          <div style={{ fontWeight: 700, color: C.pu, fontSize: 13 }}>
            🔔 {pendingReqs.length} pending maintenance request
        ...
        {user.role === "field" ? (
          ...
          <>
            <SC
              ...
              color={C.sub}
              icon="📝"
              onClick={perms.jobs_build ? () => onNav("buildjobs") : undefined}
            />
            <SC
              label="Approved"
            ...
            <SC
              ...
              icon={low.length > 0 ? "🚨" : "✅"}
              onClick={perms.inv_view ? () => onNav("inventory") : undefined}
            />
            <SC
              label="Pending Requests"
              value={pendingReqs.length}
              color={pendingReqs.length > 0 ? C.pu : C.gr}
              icon="🔧"
              onClick={() => onNav("requests")}
            />
          </>
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          >
            ...
            {jobs
              .filter((j) =>
                user.role === "field" ? j.assignedTo === user.id : true,
              )
            ...
            {jobs.filter((j) => j.status !== "completed").length === 0 && (
              <p style={{ color: C.gr, fontSize: 12, margin: 0 }}>
                ✅ No active jobs.
              </p>
            )}
          </div>
