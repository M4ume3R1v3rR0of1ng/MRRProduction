...
export default function PullInventory({
  ...
  const myJobs = isField
    ? jobs.filter((j) => j.assignedTo === user.id && j.status !== "draft")
    : jobs
  ...
  const confirmPull = async () => {
    ...
    for (const item of updItems) {
      ...
      if (!res) {
        showToast(
          `Not enough warehouse stock available for ${item.iname}. Please review staging allocations.`,
          "error",
        );
        ok = false;
        break;
    ...
    try {
      const jobRes = await supabase
        .from("jobs")
        .update({ status: "active", items: updItems })
        .eq("id", sel.id);
      ...
      showToast(
        "Materials successfully pulled from warehouse staging.",
        "success",
      );
      setModal(null);
      setPullQtys({});
  ...
  const confirmReturn = async () => {
    ...
    try {
      const jobRes = await supabase
        .from("jobs")
        .update({ status: "completed", completedAt, items: updItems })
        .eq("id", sel.id);
      if (jobRes.error) throw jobRes.error;
      ...
      setJobs((p) => p.map((j) => (j.id === sel.id ? updatedJob : j)));
      showToast(
        "Job logistics completed. Generating material manifest report.",
        "success",
      );
      setModal(null);
      setRetQtys({});
      ...
      showToast(
        `Database Error: Could not process return & completion. ${err.message}`,
        "error",
      );
    } finally {
  ...
  const syncBadge = (job) => {
    ...
    if (job.syncStatus === "synced")
      return <Bdg color="sky">☁️ AccuLynx Synced</Bdg>;
    if (job.syncStatus === "failed")
      return <Bdg color="red">⚠️ Sync Failed</Bdg>;
    if (job.syncStatus === "manual")
      return <Bdg color="amber">📋 Configure Sync</Bdg>;
    return null;
  };
  ...
  return (
    <div>
      ...
      {isField && myJobs.some((j) => j.newForAssigned) && (
        <div
          style={{
            background: C.tB,
        ...
        >
          🎉 You have new jobs assigned to you!
        </div>
      )}
      {myJobs.length === 0 && (
        <div
        ...
        >
          {isField
            ? "No jobs assigned yet. Your Production Coordinator will assign jobs here."
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {myJobs.map((job) => {
          ...
          const totalCost = job.items.reduce(
            (s, i) => s + (i.pulled - i.returned) * i.priceAtPull,
            0,
          );
          return (
            <div
            ...
            >
                  ...
                  {job.status === "completed" && (
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      <Btn
                        v="green"
                        sz="sm"
                        onClick={() => generatePDF(job, users, activeLogo)}
                      >
                        📄 PDF
                      </Btn>
                      <Btn v="sky" sz="sm" onClick={() => setSyncModal(job)}>
                        ☁️ Sync Status
                      </Btn>
                    </div>
                  )}
                ...
                {job.items.slice(0, 6).map((item) => (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10, color: C.sub }}>
                      {item.pulled > 0
                        ? `${item.pulled - item.returned} used`
                        : `${item.planned} ${item.unit} planned`}
                    </div>
                  </div>
                ))}
                {job.items.length > 6 && (
              ...
              {perms.inv_pricing_view &&
                ...
                totalCost > 0 && (
                  ...
                  >
                    <span
                      style={{ fontWeight: 900, fontSize: 15, color: C.gr }}
                    >
                      Total: {fm(totalCost)}
                    </span>
                  </div>
      ...
      {modal === "pull" && sel && (
        ...
        >
          ...
          >
            Adjust quantities if needed. Confirm to deduct from warehouse
            inventory (FIFO).
          </div>
          <table
            style={{
          ...
          >
            ...
            <tbody>
              {sel.items.map((item) => {
                const avail = tot(
                  inv.find((i) => i.id === item.iid) || { batches: [] },
                );
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: "9px 10px" }}>
                      <Inp
                        type="number"
                        value={pullQtys[item.iid] ?? item.planned}
                        min="0"
                        max={avail}
                        onChange={(e) =>
                          setPullQtys((p) => ({
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            >
              {pulling
                ? "⏳ Allocation Sync In Progress..."
                : "✅ Confirm Pull from Warehouse"}
            </Btn>
          </div>
        </Modal>
      )}
      ...
      {modal === "return" && sel && (
        ...
        >
          ...
          >
            Enter quantities being returned. PDF report + AccuLynx sync will
                  ...
                  >
                    {fm(
                      sel.items
                        ...
                        .reduce((s, i) => {
                          const ret = Math.min(
                            parseFloat(retQtys[i.iid]) || 0,
                            i.pulled,
                          );
                          return s + (i.pulled - ret) * i.priceAtPull;
                        }, 0),
                    )}
                  </td>
                </tr>
        ...
        >
              ...
              {sel.items.map((item) => (
                <tr key={item.iid} style={{ borderTop: `1px solid ${C.lg}` }}>
                  ...
                  <td
                    style={{
                      padding: "8px 10px",
                      color: item.returned > 0 ? C.am : C.sub,
                    }}
                  >
                    {item.returned}
                  </td>
                  ...
                  {perms.inv_pricing_view && (
                    ...
                    >
                      {item.pullCost > 0
                        ? fm((item.pulled - item.returned) * item.priceAtPull)
                        : "—"}
                    </td>
                  )}
                </tr>
          ...
          {sel.status === "completed" && (
            ...
            >
              <Btn
                v="green"
                onClick={() => generatePDF(sel, users, activeLogo)}
              >
                📄 PDF
              </Btn>
              <Btn v="sky" onClick={() => setSyncModal(sel)}>
                ☁️ AccuLynx Sync
              </Btn>
            </div>
          )}
      ...
      {syncModal && (
        <Modal
          title={`AccuLynx Sync — ${syncModal.po}`}
        ...
        >
          <div style={{ marginBottom: 14 }}>
            {syncModal.syncStatus === "synced" && (
              ...
              >
                <div style={{ fontWeight: 700, color: C.sl, marginBottom: 4 }}>
                  ☁️ Successfully Synced to AccuLynx
                </div>
            ...
            {syncModal.syncStatus === "failed" && (
              ...
              >
                <div style={{ fontWeight: 700, color: C.rd, marginBottom: 4 }}>
                  ⚠️ Sync Failed
                </div>
                <div style={{ fontSize: 12, color: C.sub }}>
                  {syncModal.syncNote}
            ...
            {(syncModal.syncStatus === "manual" || !syncModal.syncStatus) && (
              ...
              >
                <div style={{ fontWeight: 700, color: C.am, marginBottom: 4 }}>
                  📋 Auto-Sync Not Configured
                </div>
                <div style={{ fontSize: 12, color: C.navy }}>
                  Configure AccuLynx in Settings → AccuLynx to enable automatic
                  document upload and cost entry.
                </div>
              </div>
            )}
          </div>
