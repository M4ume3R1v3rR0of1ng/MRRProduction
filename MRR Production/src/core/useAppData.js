...
export function useAppData() {
  ...
  async function load() {
    ...
    try {
      ...
      const [ax] = await Promise.all([storage.get("mrr-v7-acculynx").catch(() => null)]);
      ...
      await Promise.all([
        ...
        (async () => {
          const { data, error } = await supabase
            ...
            .eq("company_id", activeCompanyId);
          if (error) {
            failedTables.push("Maintenance Requests");
            setReqs([]);
        ...
        (async () => {
          const { data, error } = await supabase
            ...
            .eq("company_id", activeCompanyId);
          if (error) {
            failedTables.push("Trailer Assignments");
            setJobTrailers([]);
        ...
        (async () => {
          ...
          if (error) failedTables.push("Permissions");
            ...
            data.forEach((row) => {
              ...
              formattedRolePerms[row.role] = {
                ...(DEFAULT_ROLE_PERMS[row.role] || {}),
                ...row.permissions,
              };
            });
  ...
  useEffect(() => {
    if (!loading) storage.set("mrr-v7-userov", JSON.stringify(userOverrides)).catch(() => {});
  ...
  useEffect(() => {
    ...
    const refetchPerms = async () => {
      ...
      const formattedOv = {};
      (ov || []).forEach((row) => {
        formattedOv[row.user_id] = row.overrides;
      });
  ...
  const newJobsForMe = useMemo(
    () =>
      curUser
        ? jobs.filter((j) => j.newforassigned && (j.assignedto || j.assignedTo) === curUser.id)
            .length
        : 0,
    [jobs, curUser],
  );
  const jobsAwaitingCloseCount = useMemo(
