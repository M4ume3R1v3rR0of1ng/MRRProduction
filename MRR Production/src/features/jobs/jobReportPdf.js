...
export async function buildJobReportPdf(job, users, activeLogo, inv = [], company = null) {
  ...
  const meta = [
    ...
    [
      "DATE COMPLETED",
      m.completedAt.toLocaleDateString("en-US", {
        weekday: "long",
        year: "numeric",
        month: "long",
        day: "numeric",
  ...
  for (const cat of m.categories) {
    ...
    body.push([
      {
        content: "Category Subtotal:",
        colSpan: 6,
        styles: { halign: "right", fontStyle: "italic", fillColor: ROW_BG },
      },
      {
        content: fp(cat.subtotal),
        styles: { halign: "right", fontStyle: "bold", textColor: BLUE, fillColor: ROW_BG },
      },
    ]);
  }
  ...
  const totalRow = (label, value, styles = {}) => [
    {
      content: label,
      colSpan: 6,
      styles: { halign: "right", fontStyle: "bold", fillColor: ROW_BG, ...styles },
    },
    {
      content: value,
      styles: { halign: "right", fontStyle: "bold", fillColor: ROW_BG, ...styles },
      ...
      [
        {
          ...
          colSpan: 6,
          styles: { fillColor: NAVY, textColor: WHITE, fontStyle: "bold", fontSize: 11 },
        },
        {
          content: fp(m.totalWithTax),
          styles: {
            fillColor: NAVY,
            textColor: AMBER,
            fontStyle: "bold",
