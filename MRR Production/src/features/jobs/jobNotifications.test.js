...
const { shouldNotifyJobMove, buildJobMoveEmail, notifyJobMove, DEFAULT_JOB_NOTIFICATIONS } =
  await import("./jobNotifications");
...
describe("buildJobMoveEmail", () => {
  ...
  it("escapes user-entered job text so a crafted name can't inject markup", () => {
    const m = buildJobMoveEmail("approved", {
...
describe("notifyJobMove", () => {
  ...
  it("does not send when the job has no assignable supervisor", async () => {
    const send = vi.fn();
    const noAssignee = await notifyJobMove({
      transition: "approved",
      job: { ...job, assignedto: null },
      users,
      prefs: { approved: true },
      send,
    });
    expect(noAssignee.reason).toBe("no-supervisor-email");
    const noEmail = await notifyJobMove({
      transition: "approved",
      job: { ...job, assignedto: "sup2" },
      users,
      prefs: { approved: true },
      send,
    });
  ...
  it("reads assignedTo (camelCase) as well as assignedto", async () => {
    ...
    const res = await notifyJobMove({
      transition: "active",
      job: { ...job, assignedto: undefined, assignedTo: "sup1" },
      users,
      prefs: { active: true },
      send,
    });
