...
export default function PullInventory({
  ...
  const toggleJobTrailer = async (jobId, trailerId) => {
    ...
    const notifySupervisorOfTrailerChange = async (action) => {
      ...
      showToast(
        `${assignedUser?.name || t.pullSupervisor} ${action === "added" ? t.pullTrailerAddedMsg : t.pullTrailerRemovedMsg}`,
        "success",
      );
    };
  ...
  const isOpenJob = (j) =>
    j && j.status !== "draft" && j.status !== "completed" && j.status !== "closed";
  ...
  const confirmReturn = async () => {
    ...
    try {
      ...
      newInv = inv.map((i) =>
        changedBatches.has(i.id) ? { ...i, batches: changedBatches.get(i.id) } : i,
      );
  ...
  const handleStagePhoto = async (phase, base64Data) => {
    ...
    try {
      const url = base64Data
        ? await uploadPhotoToBucket("job-attachments", user.companyId, sel.id, base64Data)
        : null;
      ...
      if (url) {
        showToast(
          `${phase === "before" ? t.pullBefore : t.pullAfter} ${t.pullPhotoSynced}`,
          "success",
        );
      }
    } catch (err) {
  ...
  const currentJobPhotos = sel
    ? { before: sel.photo_before_url || null, after: sel.photo_after_url || null }
    ...
    <div>
      ...
      >
        {[
          {
            id: "all",
            label: t.pullFilterAllShort,
            full: t.pullFilterAll,
            count: statusCounts.all,
          },
          {
            id: "approved",
            label: t.pullFilterApprovedShort,
            full: t.pullFilterApproved,
            count: statusCounts.approved,
          },
          {
            id: "active",
            label: t.pullFilterActiveShort,
        ...
        {dayGroups.map(([day, dayJobs]) => (
          <Fragment key={day || "undated"}>
            ...
            {dayJobs.map((job) => {
              ...
              const isNew =
                job.newforassigned && (job.assignedto === user.id || job.assignedTo === user.id);
              ...
              const totalCost = currentItems.reduce(
                (s, i) =>
                  s + (i ? ((i.pulled || 0) - (i.returned || 0)) * (i.priceAtPull || 0) : 0),
                0,
              );
            ...
            >
              ...
              <tbody>
                {(Array.isArray(sel.items) ? sel.items : sel.materials || []).map((item) => {
                  ...
                  const actual = Number.isNaN(parsedActual)
                    ? item.planned || item.qty || 0
                    : parsedActual;
      ...
      {modal === "edit" && sel && perms.jobs_edit_pull && (
        <Modal
          title={`${t.pullEditJob} — ${sel.po}`}
          onClose={() => {
            if (!savingEdit) setModal(null);
          }}
          wide
        >
      ...
      {modal === null && sel && (
        <Modal
          title={`${sel.po || "No PO"} — ${sel.title || sel.name}`}
          onClose={() => setSel(null)}
          wide
        >
          ...
          >
            {[
              ...
              [
                ...
                <Bdg color={(jSC[sel.status] || { c: "gray" }).c}>
                  {(jSC[sel.status] || { l: sel.status }).l}
                </Bdg>,
              ],
              ["PO", sel.po || "—"],
              [
        ...
        (() => {
          ...
          return (
            <JobHandoff
              ...
              message={
                pulled
                  ? t.pullHandoffPulledMsg
                  : canFollow
                    ? t.pullHandoffCompletedMsg
                    : t.pullHandoffCompletedMsgField
              }
              actionLabel={
                canFollow ? (pulled ? t.pullHandoffSeeActive : t.pullHandoffSeeInBuild) : null
              }
              onGo={
                canFollow
                  ? () => {
                      ...
                      setHandoff(null);
                      onShowJobIn?.(
                        pulled ? "pull" : "buildjobs",
                        j.id,
                        pulled ? t.pullJustPulled : t.pullJustCompleted,
                      );
                    }
                  : null
              }
