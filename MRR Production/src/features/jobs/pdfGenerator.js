...
import { newestPrice } from "@/shared/utils/helpers";
...
export function buildJobReportModel(job, users, inv = [], company = null) {
  ...
  // is the PRODUCT's name (Steadwerk), which is the same for everyone.
  ...
  const rawRate = Number(company?.branding?.taxRate);
  const SALES_TAX_RATE = Number.isFinite(rawRate) && rawRate >= 0 ? rawRate : 0.07;
  // parseFloat(toFixed) drops the float artifact (0.07 * 100 = 7.000000000000001) and
    ...
    <body>
      ...
      <table>
        ...
        <tfoot>
          ...
          <tr style="background:#F1F5F9">
            <td colspan="6" style="padding:10px 14px;font-weight:700;text-align:right">${escapeHtml(taxLabel)} (${taxPct}%):</td>
      ...
      <p style="margin-top:40px;font-size:11px;color:#94A3B8;text-align:center;border-top:1px solid #E5E7EB;padding-top:16px">Generated by Steadwerk · ${new Date().toLocaleString()}</p>
    </body>
    </html>
  `;
