...
| 37  | `37_maintenance_service_details.sql`          | Persists what "Complete Service" collects (service type, date, performed-by, cost) onto the ticket itself, instead of losing it once the request closes.                                          |
| 38  | `38_complete_service_notifies_driver.sql`     | `complete_maintenance_service()` also flips `newforrequester`, so the driver whose vehicle was serviced sees the same "your vehicle" dashboard alert a new request already gives them.             |
| 39  | `39_oil_due_push_notices.sql`                 | Storage for the oil-change push notification feature: `device_push_tokens` (APNs tokens) and `oil_due_notices` (default-deny, service-role only).                                                  |
| 40  | `40_oil_due_notices_driver_visibility.sql`    | Adds `driver_id` to `oil_due_notices` and a read-only policy so the assigned driver can see their own notice.                                                                                       |
| 41  | `41_chat_message_history.sql`                 | `chat_messages` — gives the Steadwerk Assistant a memory; durable, per-user chat history instead of an in-memory thread lost on reload.                                                             |
